...
/* * ***  FETCHING VARIABLES ********************************************************************* */
$_SESSION['TOPMENU'] = ($_SESSION['TOPMENU'] == 'author') ? $_SESSION['TOPMENU'] : 'author';
//paging code
...
if ($action != '' && $action != 'tc') {
    ...
    switch ($action) {
        ...
        case 'story':
            ...
            if (isset($_POST['search']) && $_POST['search'] == 'bydata') {
                ...
                if (isset($_POST['searchByContype']) && $_POST['searchByContype'] != '') {
                    $searchDataArray['content.contype_id'] = $_POST['searchByContype'];
                    $searchData .= 'searchByContype=' . $_POST['searchByContype'] . '&';
                }
  ...
  if ($action == 'a' || $action == 'm') {
    header("Location: " . CMSSITEPATH . "/author/display.php?msg=$msg");
    echo '<script>';
    echo 'window.location="' . CMSSITEPATH . '/author/display.php?msg=' . $msg . '"';
    echo '</script>';
  }
  if ($action != 'tc' && $action != 'graph') {
    $jsonStr = "{'msg':'$msg','status':'$status','action':'$action','id':'$id','module':'" . strtolower($_SESSION['TOPMENU']) . "'}";
    echo $jsonStr;
  }
} else {
  ?>
  <div class="module_<?php echo strtolower($_SESSION['TOPMENU']); ?>">
  ...
  $modulePath = CMSSITEPATH . "/" . $_SESSION['TOPMENU'];
  ...
  ?>
      <script type="text/javascript">
        if(typeof(GlobalSearch)!="undefined") {
          GlobalSearch.addCount("<?php echo strtolower($_SESSION['TOPMENU']); ?>", "<?php echo $total; ?>");
        }
      </script>
  <?php
