...
function checkavailable(){
    ...
    if(username != '' ){
		...
		$.post('availibility.php',{'val':username,'id':id},function(response){ 
            if(response != 0)
            {
                $('#usernameid').val();
...
function checkemailexist()
{
    ...
    if(flag!=1)
    {
        ...
        $('label#email_error').hide();
        $.post('availibility.php',{'email':email,'id':id},function(response) { 
            if(response != 0)
            {
                $('#emailid').val();
...
function checkauthnameexist() {
	...
	if(username != '' && by_line!=0){
		...
		$.post('availibility.php', {'authorname':username, 'id':id, 'by_line':by_line_val}, function(response) { 
            if(response != 0)
            {
                $('#usernameid').val();
