...
class Securimage {
   ...
   * @var string
   */
  var $charset = 'ACDEFGHKLMNPRSTUVWYZ2345679abcdefghijkmnpqrstyuvwxyz';
  ...
  function saveData()
  {
	...
	setcookie("securimage_code_value", sha1(strtolower($this->code)),(int)'','/');//, 'beta.seems.timescity.com'
  }
  ...
  /*  if ( isset($_SESSION['securimage_code_value']) && !empty($_SESSION['securimage_code_value']) ) {
	...
	if ( isset($_COOKIE['securimage_code_value']) && !empty($_COOKIE['securimage_code_value']) ) {
      if ($_COOKIE['securimage_code_value'] == sha1(strtolower(trim($this->code_entered))) ) {
	 
        $this->correct_code = true;
        $_COOKIE['securimage_code_value'] = '';
      } else {
        $this->correct_code = false;
      }
    } else {
  ...
  {
    if (isset($_COOKIE['securimage_code_value']) && !empty($_COOKIE['securimage_code_value'])) {
      return $_COOKIE['securimage_code_value'];
    } else {
      return '';
    }
  }
